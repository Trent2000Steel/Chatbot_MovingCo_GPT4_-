...
const stripeLink = "https://your-stripe-checkout-link.com"; // Replace with your actual Stripe Checkout link
...
export default async function handler(req, res) {
  ...
  const userId = ip; // crude session logic
  ...
  // Trigger contact info mode only after CTA click
  ...
  if (isInContactFlow) {
    ...
    if (!hasDrop) {
      global.contactMode[userId] = false; // reset mode after full collection
      return res.status(200).json({
  ...
  // Handle "more questions" fallback
...
If the user says "Yes, Reserve My Move", ask for:
- Name
- Email
- Phone
- Pickup address
...
Only after getting all 5, give them this Stripe link:
${stripeLink}

Do not ask for contact info until they confirm. Keep tone sharp and confident.
`.trim();
